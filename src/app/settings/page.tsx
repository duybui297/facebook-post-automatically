...
export default function Settings() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isEditing) {
      updatePages(pages.map(p => p.id === isEditing ? { id: isEditing, name, url, pageToken } : p));
      setIsEditing(null);
    } else {
      updatePages([...pages, { id: Date.now().toString(), name, url, pageToken }]);
    }
  ...
  const handleDelete = (id: string) => {
    if (confirm('Are you sure you want to delete this fanpage?')) {
    ...
    <div className="container" style={{ minHeight: 'calc(100vh - 80px)', display: 'flex', flexDirection: 'column', padding: '4rem 0' }}>
      <div style={{ textAlign: 'center', marginBottom: '3rem' }} className="animate-fade-in stagger-1">
        ...
        <p style={{ color: 'var(--muted)', fontSize: '1.25rem', maxWidth: '600px', margin: '0 auto' }}>
          Configure your Facebook Fanpages and Page Access Tokens.
        </p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '3rem', width: '100%', maxWidth: '1000px', margin: '0 auto' }} className="animate-fade-in stagger-2">
        ...
        <div className="glass-panel" style={{ padding: '2.5rem', alignSelf: 'start' }}>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="input-group">
              <label className="input-label" htmlFor="url">Facebook Page URL</label>
        ...
        <div className="glass-panel" style={{ padding: '2.5rem' }}>
          ...
          {isLoading ? (
            <div style={{ textAlign: 'center', padding: '3rem 1rem', color: 'var(--muted)' }}>Loading…</div>
          ) : pages.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '3rem 1rem', color: 'var(--muted)', background: 'rgba(0,0,0,0.2)', borderRadius: 'var(--radius-md)', border: '1px dashed var(--border)' }}>
              No fanpages yet. Add one to get started!
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {pages.map((page) => (
                <div key={page.id} style={{ padding: '1.25rem', background: 'rgba(0,0,0,0.2)', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', transition: 'all 0.2s' }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem', paddingLeft: '1rem', flexShrink: 0 }}>
                    <button onClick={() => handleEdit(page)} style={{ background: 'transparent', border: 'none', color: 'var(--primary)', cursor: 'pointer', fontSize: '0.9rem', fontWeight: 600, padding: '0.5rem' }}>Edit</button>
                    <button onClick={() => handleDelete(page.id)} style={{ background: 'transparent', border: 'none', color: 'var(--accent)', cursor: 'pointer', fontSize: '0.9rem', fontWeight: 600, padding: '0.5rem' }}>Delete</button>
                  </div>
                </div>
              ))}
            </div>
